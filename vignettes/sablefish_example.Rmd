...
# and getExpansion_2() or get_pacfin_expansions().
expansion <- 0.95
```
...
bds_cleaned <- cleanPacFIN(
  ...
  # Add a stratification column based upon state and gear
  dplyr::mutate(
...
length and are considered independent of the lengths in the model).  Historically, two
...
The `tableSample()` can be used to create sample tables. Depending upon the data object
passed, this function will create either a table of the samples of lengths and ages by 
year and stratification (default `state` and `geargoup`) or a table of the trips,
samples, and input sample size by year and fleet.

```{r sample-size-table, eval = FALSE}
age_and_length_samples <- tableSample(
  Pdata = bds_cleaned,
...
```{r length-input-sample-size-table, eval = FALSE}
length_trips_input_sample_sizes <- tableSample(
  Pdata = length_comps_long,
  savedir = getwd(),
  verbose = TRUE
)
```
...
```{r age-input-sample-size-table, eval = FALSE}
age_trips_input_sample_sizes <- tableSample(
  Pdata = age_comps_long,
  savedir = getwd(),
  verbose = TRUE
)
```
